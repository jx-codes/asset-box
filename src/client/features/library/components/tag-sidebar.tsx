...
export function TagSidebar({ tags, assetCount }: { tags: Tag[]; assetCount: number }) {
  ...
  return (
    <aside className="hidden w-60 flex-none flex-col border-r bg-sidebar md:flex">
      ...
      <Separator />
      <nav className="min-h-0 flex-1 overflow-y-auto p-2" aria-label="Asset tags">
        <button
          type="button"
          className={`flex w-full items-center justify-between px-2 py-1.5 text-left text-sm ${filter.tag === "all" ? "bg-foreground text-background" : "hover:bg-black/5"}`}
